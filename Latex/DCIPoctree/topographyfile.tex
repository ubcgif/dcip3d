...
\end{fileExample}
%
\textbf{NOTE}: Although the cells above the topographic surface are made inactive, they must still be included in the \fileName{model} file as if they are a part of the model. For input model files these cells can be assigned any value. For conductivity models these air cells are typically assigned a value of $10^-8$ S/m. The recovered model produced by inversion program also includes the cells that are excluded from the model, but these cells will have unrealistic values as an identifier (e.g. \codeName{-100}).